...
def take_test(test_id):
    ...
    questions = []
    for q in qs_db:
        qd = serialize_doc_for_template(q)
        questions.append(qd)

    test_serialized = serialize_doc_for_template(test_db)
    return render_template('take_test.html', test=test_serialized, questions=questions)
...
@app.route('/student/download_report')
def download_student_report():
    ...
    # Fetch submissions and related tests
    ...
    total_possible = 0.0
    for s in submissions:
        total_scored += float(s.get('score', 0))
        total_possible += float(s.get('possible', 0))
    percentage = round((total_scored / total_possible) * 100, 2) if total_possible > 0 else 0

    # Generate PDF
    buffer = BytesIO()
    pdf = canvas.Canvas(buffer, pagesize=A4)
    pdf.setTitle(f"{user['username']}_report")

    y = 800
    pdf.setFont("Helvetica-Bold", 16)
    pdf.drawString(50, y, f"Student Report — {user['username']}")
    y -= 30
    pdf.setFont("Helvetica", 12)
    pdf.drawString(50, y, f"Total Score: {total_scored} / {total_possible} ({percentage}%)")
    y -= 30
    ...
    if not submissions:
        ...
        for sub in submissions:
            ...
            if test:
                ...
                pdf.drawString(50, y, f"Test: {title} — Score: {score}/{possible}")
                y -= 20
                if y < 50:
                    pdf.showPage()
                    y = 800
